const {
  BaseCapability,
} = require("../../../packages/capability-sdk/src/baseCapability");
const {
  createCapabilityResult,
} = require("../../../packages/capability-sdk/src/capabilityResult");

class BookingCapability extends BaseCapability {
  async canHandle(context) {
    const i = context.intelligence?.selected;
    if (i?.capabilityId === "booking")
      return { confidence: i.confidence || 0.97, reason: i.reason };
    return { confidence: 0 };
  }
  ...
  async execute(context) {
    ...
    if (selected.intent === "booking.customer_field_edit") {
      ...
      if (!["name", "phone", "email"].includes(field))
        return createCapabilityResult({
          handled: true,
          reply:
            "Tell me whether you want to change the booking name, phone, or optional email.",
          statePatch: {
            activePlugin: "booking",
            capabilityState: { booking: previous },
          },
        });
      const rawValue = amendment.rawValue;
      const metadata = { ...(previous.metadata || {}) };
      if (rawValue == null || String(rawValue).trim() === "") {
        metadata.pendingFieldEdit = {
          field,
          resumePendingField: previous.pendingField || null,
        };
        ...
        return createCapabilityResult({
          ...
          reply,
          responseModel: {
            intent: "GENERIC_BOOKING_FIELD_EDIT_NEEDS_VALUE",
            payload: { legacyText: reply, pendingField: field },
          },
          statePatch: {
            activePlugin: "booking",
            lastIntent: "booking_field_edit_needs_value",
            capabilityState: { booking: { ...previous, metadata } },
          },
        });
      }
      const parsed = engagement.parseField(
        field,
        rawValue,
        fieldOptions(field, config),
      );
      if (!parsed.valid) {
        metadata.pendingFieldEdit = {
          field,
          resumePendingField:
            metadata.pendingFieldEdit?.resumePendingField ||
            previous.pendingField ||
            null,
        };
        ...
        return createCapabilityResult({
          ...
          reply,
          responseModel: {
            intent: "GENERIC_BOOKING_FIELD_EDIT_INVALID",
            payload: {
              legacyText: reply,
              pendingField: field,
              unchanged: true,
          ...
          statePatch: {
            activePlugin: "booking",
            lastIntent: "booking_field_edit_invalid",
            capabilityState: { booking: { ...previous, metadata } },
          },
        });
      }
      const resumePendingField =
        metadata.pendingFieldEdit?.resumePendingField ||
        previous.pendingField ||
        null;
      delete metadata.pendingFieldEdit;
      if (previous.status === "completed" && previous.bookingId) {
        const record = await booking.updateDetails(previous.bookingId, {
          [field]: parsed.value,
        });
        ...
        return createCapabilityResult({
          ...
          reply,
          responseModel: {
            intent: "GENERIC_BOOKING_CUSTOMER_FIELD_UPDATED",
            payload: {
              legacyText: reply,
              bookingId: record.id,
              field,
          ...
          statePatch: {
            activePlugin: "booking",
            lastIntent: "booking_customer_field_updated",
            capabilityState: {
              booking: {
                ...previous,
                slots: { ...(previous.slots || {}), [field]: parsed.value },
                metadata,
              },
            },
          },
      ...
      const flow = engagement.normalizeState(
        {
          kind: config.mode || "booking",
          status: previous.status || "collecting",
          items: previous.items || [],
          fields: {
            ...(previous.slots || previous.fields || {}),
            [field]: parsed.value,
          },
          pendingField: resumePendingField,
          metadata,
        },
        config,
      );
      const missing = engagement.nextMissing(
        requiredFields(config),
        flow.fields,
      );
      flow.pendingField = missing || "confirmation";
      flow.status = missing ? "collecting" : "ready";
      const continuation = missing
        ? engagement.prompt(missing, config, context.language)
        : config.confirmPrompt || "If everything is correct, say confirm.";
      ...
      return createCapabilityResult({
        ...
        responseModel: {
          ...
          payload: {
            ...
            field,
            value: parsed.value,
            pendingField: flow.pendingField,
          },
        },
        statePatch: {
          activePlugin: "booking",
          lastIntent: "booking_customer_field_updated",
          capabilityState: { booking: stateShape(flow) },
        },
      });
    }
    ...
    if (selected.intent === "booking.cancel_none") {
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_CANCEL_NONE",
          payload: { legacyText: reply },
        },
        statePatch: {
          activePlugin: "booking",
          lastIntent: "booking_cancel_none",
          capabilityState: { booking: {} },
        },
      });
    }
    if (selected.intent === "booking.cancel_selection_required") {
      const bookings =
        extracted.bookings || previous.metadata?.cancelChoices || [];
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_CANCEL_SELECTION_REQUIRED",
          payload: { legacyText: reply, bookings },
        },
        statePatch: {
          activePlugin: "booking",
          lastIntent: "booking_cancel_selection_required",
          capabilityState: {
            booking: {
              status: "cancel_selection",
              pendingField: "bookingId",
              metadata: { cancelChoices: bookings },
            },
          },
        },
      });
    ...
    if (selected.intent === "booking.cancel_request") {
      ...
      if (!bookingId)
        return createCapabilityResult({
          handled: true,
          reply:
            "I could not find an active booking for this customer to cancel.",
          statePatch: { lastIntent: "booking_cancel_missing" },
        });
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_CANCELLED",
          payload: { legacyText: reply, record },
        },
        statePatch: {
          activePlugin: "booking",
    ...
    if (selected.intent === "booking.reschedule_request")
      return rescheduleResult(booking, previous, extracted, config);
    if (selected.intent === "booking.items_amendment_request") {
      const requested = (extracted.offeringIds || [])
        .map((id) => offering.getById(id))
        .filter(Boolean)
        .map((item) => ({
          id: item.id,
          name: item.name,
          ...
          metadata: { durationMinutes: item.durationMinutes || null },
        }));
      const action = extracted.amendmentAction || "replace";
      const proposal = {
        type: "items_amendment",
        action,
        items: requested,
        requestedAt: new Date().toISOString(),
      };
      const persisted = previous.bookingId
        ? await booking.proposeAmendment(previous.bookingId, proposal)
        : null;
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_ITEMS_AMENDMENT_REQUIRES_APPROVAL",
          payload: {
            legacyText: reply,
            proposal,
        ...
        statePatch: {
          activePlugin: "booking",
          lastIntent: "booking_items_amendment_requires_approval",
          capabilityState: {
            booking: {
              ...previous,
              metadata: {
                ...(previous.metadata || {}),
                proposedItemsAmendment: proposal,
              },
            },
          },
        },
    ...
    if (selected.intent === "booking.add_item_clarify") {
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_ITEM_CLARIFICATION",
          payload: { legacyText: reply, choices },
        },
        statePatch: {
          activePlugin: "booking",
          lastIntent: "booking_add_item_clarify",
          capabilityState: {
            booking: {
              ...previous,
              metadata: {
                ...(previous.metadata || {}),
                pendingOfferingChoices: choices,
              },
            },
          },
        },
    ...
    let flow = engagement.normalizeState(
      {
        ...
        status: previous.status || "collecting",
        items: previous.items || [],
        fields: previous.slots || previous.fields || {},
        pendingField: previous.pendingField || null,
    ...
    if (selected.intent === "booking.view") {
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_VIEWED",
          payload: { legacyText: reply, engagement: flow },
        },
        statePatch: {
          activePlugin: "booking",
          lastIntent: "booking_viewed",
          capabilityState: { booking: previous },
        },
      });
    }
        ...
        flow = engagement.addItem(
          flow,
          {
            ...
            metadata: {
              ...
              priceVariable: Boolean(item.pricePrefix),
            },
          },
          { quantity: 1 },
        );
    ...
    if (
      ...
      (selected.intent || "").startsWith("booking.")
    ) {
      flow = engagement.addItem(
        flow,
        {
          id: `default:${slug(config.defaultSubject)}`,
          name: config.defaultSubject,
    ...
    if (
      extracted.email &&
      flow.pendingField &&
      !has(extracted[flow.pendingField]) &&
      /^\s*(?:(?:my\s+)?email(?:\s+address)?\s*(?:is|=|:)?\s*)?[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\s*[.!]?\s*$/i.test(
    ...
    if (
      (selected.intent === "booking.continue" ||
        selected.intent === "booking.start") &&
      flow.pendingField &&
      flow.pendingField !== "confirmation"
    ) {
      const pending = flow.pendingField;
      if (
        engagement.referencesStoredDetails?.(context.message.text) ||
        engagement.referencesStoredField?.(pending, context.message.text)
      ) {
        const saved = await savedBookingFields(context);
        const patch = engagement.referencesStoredDetails?.(context.message.text)
          ? saved
          : { ...(saved[pending] ? { [pending]: saved[pending] } : {}) };
        for (const [field, value] of Object.entries(patch))
          if (requiredFields(config).includes(field) && has(value))
            flow.fields[field] = value;
    ...
    if (scheduleIssue) {
      if (scheduleIssue.field === "date") {
        delete flow.fields.date;
        delete flow.fields.time;
      } else delete flow.fields.time;
      flow.status = "collecting";
      flow.pendingField = scheduleIssue.field;
    ...
    if (selected.intent === "booking.confirm" && previous.status === "ready") {
      let record;
      try {
        record = await booking.create(legacySlots(flow, config), {
          holdId: previous.metadata?.calendarHoldId || null,
        });
      } catch (error) {
        if (
          error.code === "CALENDAR_SLOT_UNAVAILABLE" ||
          error.code === "CONFLICT"
        ) {
          delete flow.metadata.calendarHoldId;
          delete flow.metadata.calendarHoldExpiresAt;
          flow.status = "collecting";
          flow.pendingField = "time";
          const alternatives = formatAlternatives(
            error.alternatives || error.details?.alternatives || [],
          );
      ...
      flow.status = "completed";
      flow.pendingField = null;
      flow.metadata.calendarEventId = record.calendarEventId || null;
      if (record.status === "confirmed") {
        flow.metadata.calendarStatus = "confirmed";
        delete flow.metadata.calendarHoldId;
        delete flow.metadata.calendarHoldExpiresAt;
      }
      const label =
        record.status === "confirmed"
          ? `${config.confirmedLabel || "Booking request received"} — slot confirmed`
          : config.confirmedLabel || "Booking request received";
      const reply = `✅ ${label}\nReference: ${record.id}\n${engagementSummary(flow, config)}`;
      return createCapabilityResult({
        handled: true,
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_CREATED",
          payload: { legacyText: reply, record, engagement: flow },
        },
        statePatch: {
          activePlugin: "booking",
          pendingQuestion: null,
          lastIntent: "booking_created",
          capabilityState: { booking: stateShape(flow, record.id) },
        },
        events: [
          { name: "booking.created.v1", payload: { bookingId: record.id } },
        ],
      });
    }
    ...
    if (!missing) {
      ...
      if (held?.status === "unavailable" && flow.metadata.alternativeTime) {
        const alternativeSlots = {
          ...slots,
          date: flow.metadata.alternativeDate || slots.date,
          time: flow.metadata.alternativeTime,
        };
      ...
      flow.pendingField = "confirmation";
      const intro =
        addedNames.length && selected.intent === "booking.add_item"
      ...
      return createCapabilityResult({
        ...
        reply,
        responseModel: {
          intent: "GENERIC_BOOKING_READY",
          payload: { legacyText: reply, engagement: flow },
        },
        statePatch: {
          activePlugin: "booking",
          pendingQuestion: "confirmation",
          lastIntent: "booking_ready",
          capabilityState: { booking: stateShape(flow) },
        },
      });
    }
    ...
    flow.status = "collecting";
    flow.pendingField = missing;
    let basePrompt = engagement.prompt(missing, config, context.language);
    if (missing === "time" && flow.metadata.timeWindow)
      basePrompt = `You chose the ${flow.metadata.timeWindow}; please give an exact time. ${basePrompt}`;
    const selectedIntro = addedNames.length
      ? `${addedNames.join(", ")} selected 👍\n`
      : "";
    const currentSummary = engagementSummary(flow, config);
    const saved = await savedBookingFields(context);
    const reusable =
      ["name", "phone", "email", "city", "address", "landmark"].includes(
        missing,
      ) && has(saved[missing]);
    const savedOffer = reusable
      ? `I found these saved details:\n${Object.entries(saved)
          .filter(([field]) => requiredFields(config).includes(field))
          .map(([field, value]) => `• ${pretty(field)}: ${value}`)
          .join(
            "\n",
          )}\n\nSay “use my saved details” to reuse them, or provide a new ${pretty(missing).toLowerCase()}.`
      : basePrompt;
...
async function rescheduleResult(booking, previous, extracted, config) {
  const proposedChange = {
    date: extracted.date || previous.slots?.date || null,
    time: extracted.time || null,
    constraint: extracted.afterTime ? "after" : null,
    requestedAt: new Date().toISOString(),
  };
  ...
  const persisted =
    previous.bookingId && proposedChange.time
      ? await booking.reschedule(previous.bookingId, proposedChange)
      : previous.bookingId
        ? await booking.proposeAmendment(previous.bookingId, proposedChange)
        : null;
  if (persisted?.status === "rescheduled") {
    ...
    const slots = {
      ...(previous.slots || {}),
      date: updated.slots.date,
      time: updated.slots.time,
    };
    return createCapabilityResult({
      handled: true,
      reply,
      responseModel: {
        intent: "GENERIC_BOOKING_RESCHEDULED",
        payload: { legacyText: reply, record: updated, event: persisted.event },
      },
      statePatch: {
        activePlugin: "booking",
        lastIntent: "booking_rescheduled",
        capabilityState: {
          booking: {
            ...previous,
            slots,
  ...
  if (persisted?.status === "unavailable") {
    ...
    return createCapabilityResult({
      ...
      reply,
      responseModel: {
        intent: "GENERIC_BOOKING_RESCHEDULE_UNAVAILABLE",
        payload: {
          legacyText: reply,
          proposedChange,
      ...
      statePatch: {
        activePlugin: "booking",
        lastIntent: "booking_reschedule_unavailable",
        capabilityState: { booking: previous },
      },
    });
  }
  const revision = persisted?.revision
    ? ` The stored booking is now revision ${persisted.revision}.`
    : "";
  ...
  return createCapabilityResult({
    ...
    reply,
    responseModel: {
      intent: "GENERIC_BOOKING_RESCHEDULE_REQUIRES_LIVE_CHECK",
      payload: {
        legacyText: reply,
        proposedChange,
        originalBookingId: previous.bookingId || null,
        revision: persisted?.revision || null,
      },
    },
    statePatch: {
      activePlugin: "booking",
      lastIntent: "booking_reschedule_requires_live_check",
      capabilityState: {
        booking: {
          ...previous,
          metadata: {
...
function formatAlternatives(rows) {
  if (!rows?.length) return "";
  return `. Available alternatives: ${rows.map((row) => `${row.date} at ${row.time}`).join(", ")}`;
}
function collectingResult(config, flow, field, reply) {
  return createCapabilityResult({
    handled: true,
    reply,
    responseModel: {
      intent: "GENERIC_BOOKING_COLLECTING",
      payload: { legacyText: reply, engagement: flow, pendingField: field },
    },
    statePatch: {
      activePlugin: "booking",
      pendingQuestion: field,
      lastIntent: "booking_collecting",
      capabilityState: { booking: stateShape(flow) },
    },
  });
}
function requiredFields(config) {
  return (
    config.requiredFields || ["subject", "date", "time", "name", "phone"]
  ).filter((x) => x !== "subject");
}
async function savedBookingFields(context) {
  ...
  const location =
    customer.customFields?.lastKnownLocation ||
    customer.customFields?.lastDelivery ||
    {};
  const values = {
    ...
    address: location.address || customer.customFields?.primaryAddress || null,
    landmark: location.landmark || null,
  };
  return Object.fromEntries(
    Object.entries(values).filter(([, value]) => has(value)),
  );
}
function mergeMetadata(flow, e) {
  const keys = [
    ...
    "closestTimeRequested",
  ];
  for (const key of keys) if (e[key] != null) flow.metadata[key] = e[key];
}
function availabilityNotice(flow, config) {
  if (
    !flow.metadata.availabilityRequested ||
    flow.metadata.availabilityNoticeShown
  )
    return "";
  if (config.mode === "reservation")
    return "I cannot confirm live table availability or the closest time from the current source. I have captured your requested date, time, and party size without promising a slot.";
...
function engagementSummary(flow, config) {
  ...
  else if (flow.items.length > 1) {
    lines.push("Selections:");
    for (const item of flow.items) lines.push(`• ${item.name}`);
  }
  const labels = config.labels || {};
  for (const [k, v] of Object.entries(flow.fields))
    if (k !== "subject" && has(v))
      lines.push(`${labels[k] || pretty(k)}: ${v}`);
  if (flow.metadata.preferredEndTime)
    lines.push(`Preferred time window ends: ${flow.metadata.preferredEndTime}`);
  if (flow.metadata.hairLength)
    lines.push(`Hair length: ${flow.metadata.hairLength}`);
  if (flow.metadata.referenceCode)
    lines.push(`Reference: ${flow.metadata.referenceCode}`);
  const priced = flow.items.filter((x) => Number.isFinite(Number(x.price)));
  if (
    flow.metadata.priceRequested &&
    priced.length === flow.items.length &&
    priced.length
  ) {
    const total = priced.reduce(
      (sum, x) => sum + Number(x.price) * Number(x.quantity || 1),
      0,
    );
    const from = priced.some((x) => x.metadata?.pricePrefix);
    lines.push(
      `Estimated price: ${from ? "From " : ""}Rs${total.toLocaleString("en-US")}`,
    );
    if (priced.some((x) => x.metadata?.priceVariable))
      lines.push("The final hair color price can vary with hair length.");
  }
  if (flow.metadata.durationRequested) {
    const minutes = flow.items.reduce(
      (sum, x) =>
        sum +
        Number(x.metadata?.durationMinutes || 0) * Number(x.quantity || 1),
      0,
    );
...
function legacySlots(flow, config) {
  return {
    subject:
      flow.items.map((x) => x.name).join(" + ") ||
      config.defaultSubject ||
      null,
    items: flow.items.map((x) => ({
      id: x.id,
      name: x.name,
      type: x.type,
      quantity: x.quantity,
    ...
    ...flow.fields,
    ...(flow.metadata.referenceCode
      ? { referenceCode: flow.metadata.referenceCode }
      : {}),
    metadata: flow.metadata,
  };
}
function stateShape(flow, bookingId = null) {
  return {
    status: flow.status,
    items: flow.items,
    slots: {
      ...flow.fields,
      subject: flow.items.map((x) => x.name).join(" + ") || null,
      ...(flow.metadata.referenceCode
        ? { referenceCode: flow.metadata.referenceCode }
        : {}),
    },
...
function fieldOptions(field, config) {
  if (field === "phone")
    return {
      minDigits: config.phoneValidation?.minDigits || 10,
      maxDigits: config.phoneValidation?.maxDigits || 15,
    };
  if (field === "date") return { allowPast: Boolean(config.allowPastDates) };
  if (field === "partySize") return { min: 1, max: config.maxPartySize || 100 };
  return {};
}
...
function validateBookingSchedule(context, flow) {
  ...
  const day = weekdayForDate(flow.fields.date),
    availability = context.services.availability;
  const operating = availability?.operatingDay?.(day);
  if (operating?.status === "closed")
    return {
      field: "date",
      message: `We’re closed on ${title(day)}. Please choose an open business day.`,
    };
  if (flow.fields.time) {
    const duration = Number(flow.fields.durationHours || 0);
    const checked = availability?.validateTime?.(day, flow.fields.time, {
      durationMinutes: duration ? duration * 60 : null,
      endTime: flow.metadata.preferredEndTime || null,
    });
    if (checked?.valid === false)
      return { field: "time", message: checked.message };
  }
  return null;
}
function weekdayForDate(value) {
  ...
  return [
    ...
    new Date(Date.UTC(Number(m[3]), Number(m[2]) - 1, Number(m[1]))).getUTCDay()
  ];
}
function title(value) {
  return String(value || "that day").replace(/^./, (char) =>
    char.toUpperCase(),
  );
}
